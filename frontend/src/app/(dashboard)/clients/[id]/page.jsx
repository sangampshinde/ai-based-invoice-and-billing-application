...
function isOverdue(inv) {
  return (inv.status === "sent" || inv.status === "unpaid") && inv.due_date && new Date(inv.due_date) < new Date();
}
...
export default function ClientDetailPage({ params }) {
  ...
  const stats = useMemo(() => {
    ...
    const outstanding = invoices.filter((i) => i.status !== "paid").reduce((s, i) => s + (parseFloat(i.total) || 0), 0);
    return { count, totalBilled, outstanding };
  }, [data?.stats, invoices]);
    ...
    <div>
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="accent"
            onClick={() => router.push(`/invoices/new?client=${id}`)}
          >
          ...
          <Card padding="lg">
            ...
            {invoices.length === 0 ? (
              <div className="py-10 text-center">
                ...
                <Button variant="soft" size="sm" className="mt-3" onClick={() => router.push(`/invoices/new?client=${id}`)}>
              ...
              <div className="divide-y divide-[var(--border)]">
                {invoices.map((inv) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 py-3 text-left hover:opacity-90 cursor-pointer"
                  >
                    ...
                    <StatusBadge
                      status={
                        (inv.status === "sent" || inv.status === "unpaid") && inv.due_date && new Date(inv.due_date) < new Date()
                          ? "overdue"
                          : inv.status
                      }
                    />

  ...
  const invoices = useMemo(() => {
    ...
    if (status !== "all") {
      list = list.filter((inv) => {
        const eff = inv.status === "unpaid" ? "sent" : inv.status;
        return eff === status;
      });
    }
    ...
    return list;
  }, [rawInvoices, status, search, sort]);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <Button variant="accent" onClick={() => router.push("/invoices/new")}>
        ...
        <div className="flex items-center gap-1 p-1 rounded-full bg-[var(--surface)] border border-[var(--border)] shadow-card w-fit">
          {STATUS_TABS.map((t) => (
            <button
              ...
              className={cn(
                "h-8 px-4 rounded-full text-xs font-semibold transition-colors cursor-pointer",
                status === t.key
            ...
            {invoices.map((inv) => (
              ...
              >
                ...
                <div className="flex items-center justify-end gap-1">
                  <StatusBadge status={inv.status === "unpaid" ? "sent" : inv.status} />
                  <div className="hidden group-hover:flex items-center gap-0.5 ml-1">
                    <button
                      ...
                      title="Edit"
                      className="h-7 w-7 rounded-full flex items-center justify-center text-[var(--ink-muted)] hover:bg-[var(--surface)] hover:text-[var(--ink)] cursor-pointer"
                    >
                      <Pencil size={13} />
                    </button>
                    <button
                      ...
                      title="Delete"
                      className="h-7 w-7 rounded-full flex items-center justify-center text-[var(--ink-muted)] hover:bg-[var(--surface)] hover:text-[var(--danger)] cursor-pointer"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
...
function SortHead({ label, active, order, onClick }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "inline-flex items-center gap-1 uppercase tracking-wider text-[11px] font-semibold hover:text-[var(--ink)] transition-colors w-fit cursor-pointer",

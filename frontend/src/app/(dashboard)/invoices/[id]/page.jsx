...
export default function InvoiceDetailPage({ params }) {
  ...
  const st = invoice.status === "unpaid" ? "sent" : invoice.status;
  ...
  return (
    <div className="max-w-[1100px]">
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        ...
        />
        <StatusButton
          active={invoice.status === "unpaid" || invoice.status === "sent"}
          onClick={() => setStatus.mutate({ id, status: "unpaid" })}
          icon={Send}
          label="Sent"
        />
        <StatusButton
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          {!isPaid && <PaymentReminderCard invoice={invoice} />}
...
function StatusButton({ active, onClick, icon: Icon, label, tone }) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "inline-flex items-center gap-1.5 h-8 px-3 rounded-full text-xs font-semibold border transition-colors cursor-pointer",
        active
          ? tone === "success"
    ...
    <Card padding="lg">
      ...
      <div className="flex items-start justify-between gap-4 py-6">
        <div>
          ...
          {invoice.client_address && <div className="text-xs text-[var(--ink-muted)]">{invoice.client_address}</div>}
        </div>
      ...
      <div className="ml-auto w-full max-w-[260px] mt-5 space-y-2 text-sm">
        ...
        <TotalLine label={`Tax (${Number(invoice.tax_rate || 0)}%)`} value={formatMoney(invoice.tax, currency)} />
...
function PaymentReminderCard({ invoice }) {
  ...
  async function generate() {
    ...
    setErr("");
    try {
      const res = await aiApi.paymentReminder({
  ...
  return (
    <Card padding="lg">
      ...
      <p className="text-xs text-[var(--ink-muted)] mb-3">
        Draft a polite, professional, and firm reminder email with Gemini AI.
      </p>
      ...
      {draft && (
        <div className="mt-4 rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <button
              onClick={copy}
              className="shrink-0 inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--accent-strong)] cursor-pointer"
            >

...
export default function ReportsPage() {
  ...
  const maxClient = Math.max(1, ...topClients.map((c) => c.billed || 0));

  function exportCSV() {
    const rows = [
      ["Metric", "Value"],
      ["Total revenue", totals.revenue || 0],
      ["Total expenses", totals.expenses || 0],
      ["Net profit", totals.netProfit || 0],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard label="Total Revenue" value={formatMoney(totals.revenue || 0)} icon={Wallet} accent />
        <StatCard label="Total Expenses" value={formatMoney(totals.expenses || 0)} icon={Receipt} />
        <StatCard label="Net Profit" value={formatMoney(totals.netProfit || 0)} icon={TrendingUp} />
        <StatCard label="Outstanding" value={formatMoney(totals.outstanding || 0)} icon={Clock} />
      </div>
              ...
              <ResponsiveContainer width="100%" height={180}>
                <PieChart>
                  <Pie data={statusData} dataKey="value" innerRadius={50} outerRadius={72} paddingAngle={2} stroke="none">
                    {statusData.map((s) => <Cell key={s.key} fill={STATUS_COLORS[s.key] || "var(--accent)"} />)}
                  </Pie>
              ...
              <div className="space-y-2 mt-2">
                {statusData.map((s) => (
                  <div key={s.key} className="flex items-center gap-2 text-sm">
                    <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ background: STATUS_COLORS[s.key] || "var(--accent)" }} />

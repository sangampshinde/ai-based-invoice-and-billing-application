...
export default function DashboardPage() {
  ...
  if (!stats?.invoiceCount) {
    return (
      <EmptyState
        ...
        action={
          <Button variant="accent" size="lg" onClick={() => router.push("/invoices/new")}>
  ...
  return (
    <div className="space-y-6">
      ...
      <RecentInvoices invoices={recentInvoices} onOpen={(id) => router.push(`/invoices/${id}`)} />
    </div>
  );
}
  ...
  return (
    <Card padding="lg">
      ...
      {rows.length === 0 ? (
        ...
        <div className="flex flex-col divide-y divide-[var(--border)] pt-1">
          {rows.map((inv) => (
            <button
              key={inv.id}
              onClick={() => onOpen(inv.id)}
              className="group flex items-center gap-3.5 py-3.5 px-3 -mx-3 rounded-2xl text-left hover:bg-[var(--surface-2)] transition-all"
            >

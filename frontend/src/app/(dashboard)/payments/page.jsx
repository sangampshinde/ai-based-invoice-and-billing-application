...
export default function PaymentsPage() {
  ...
  const totals = useMemo(() => {
    const total = rawPayments.reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
    const thisMonthPrefix = new Date().toISOString().slice(0, 7);
    const thisMonth = rawPayments
      .filter((p) => (p.payment_date || p.paid_on || "").startsWith(thisMonthPrefix))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6 max-w-2xl">
        <StatCard label="Total received" value={formatMoney(totals.total)} icon={Wallet} accent />
        <StatCard label="Received this month" value={formatMoney(totals.thisMonth)} icon={CreditCard} />
      </div>
          ...
          <div className="divide-y divide-[var(--border)]">
            {rawPayments.map((p) => (
              <div key={p.id} className="group grid grid-cols-2 md:grid-cols-[1fr_1.4fr_1fr_1fr_auto] gap-x-4 gap-y-1 px-5 py-4 items-center">
                <div className="text-sm text-[var(--ink-muted)] tabular">{formatDate(p.payment_date || p.paid_on)}</div>
                ...
                <div className="hidden md:block">
                  {p.method ? <Badge tone="neutral" className="capitalize">{p.method.replace('_', ' ')}</Badge> : <span className="text-xs text-[var(--ink-muted)]">—</span>}
                </div>
                <div className="text-sm font-semibold text-[var(--success)] tabular text-right">{formatMoney(p.amount)}</div>
                <button onClick={() => onDelete(p)} className="justify-self-end h-7 w-7 rounded-full flex items-center justify-center text-[var(--ink-muted)] opacity-0 group-hover:opacity-100 transition-opacity hover:bg-[var(--surface-2)] hover:text-[var(--danger)] cursor-pointer">
                  <Trash2 size={13} />
                </button>
              </div>
            ))}
...
function RecordPaymentModal({ open, onClose }) {
  ...
  const [form, setForm] = useState({ invoiceId: "", amount: "", method: "bank_transfer", paid_on: "", notes: "" });
  const [err, setErr] = useState("");
  const [saving, setSaving] = useState(false);

  const options = useMemo(
    () => (invoices || []).filter((i) => i.status !== "paid").concat((invoices || []).filter((i) => i.status === "paid")),
    [invoices]
  );

  useEffect(() => {
    if (open) {
      setForm({ invoiceId: "", amount: "", method: "bank_transfer", paid_on: new Date().toISOString().slice(0, 10), notes: "" });
      setErr("");
    }
  }, [open]);
            ...
            <div className="space-y-3">
                ...
                <select className={selectClass} value={form.invoiceId} onChange={(e) => pickInvoice(e.target.value)}>
                  ...
                  {options.map((i) => (
                    <option key={i.id} value={i.id}>
                      {i.invoice_number} · {i.client_name || "No client"} · {formatMoney(i.total, i.currency)}{i.status === "paid" ? " (paid)" : ""}
                    </option>
                  ))}
                </select>
              </Field>
              ...
              <Field label="Method">
                <select className={selectClass} value={form.method} onChange={(e) => setForm((f) => ({ ...f, method: e.target.value }))}>
                  {METHODS.map((m) => (
                    <option key={m} value={m.toLowerCase().replace(' ', '_')}>
                      {m}
                    </option>
                  ))}
                </select>

...
export default function ExpensesPage() {
  ...
  const categories = useMemo(() => {
    return Array.from(new Set(rawExpenses.map((e) => e.category).filter(Boolean)));
  }, [rawExpenses]);

  const totals = useMemo(() => {
    const total = rawExpenses.reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
    const thisMonthPrefix = new Date().toISOString().slice(0, 7);
    const thisMonth = rawExpenses
      .filter((e) => (e.date || e.expense_date || "").startsWith(thisMonthPrefix))
      .reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0);
    return { total, thisMonth };
  }, [rawExpenses]);

  const expenses = useMemo(() => {
    if (category === "all") return rawExpenses;
    return rawExpenses.filter((e) => e.category === category);
  ...
  async function onScan(e) {
    ...
    try {
      ...
      setModal({
        ...
        expense_date: res.date || new Date().toISOString().slice(0, 10),
        amount: res.total || res.subtotal || res.items?.reduce((s, i) => s + (parseFloat(i.rate) * (parseFloat(i.quantity) || 1) || 0), 0) || 0,
        notes: res.notes || (res.items?.[0]?.description ?? ""),
      });
    } catch (ex) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6 max-w-2xl">
        <StatCard label="Total expenses" value={formatMoney(totals.total)} icon={Receipt} />
        <StatCard label="This month" value={formatMoney(totals.thisMonth)} icon={Receipt} accent />
      </div>
        ...
        <div className="flex items-center gap-1 p-1 rounded-full bg-[var(--surface)] border border-[var(--border)] shadow-card w-fit mb-5 flex-wrap">
          {["all", ...categories].map((c) => (
            <button
              ...
              className={cn(
                "h-8 px-4 rounded-full text-xs font-semibold transition-colors capitalize cursor-pointer",
          ...
          <div className="divide-y divide-[var(--border)]">
            {expenses.map((exp) => (
              ...
              >
                ...
                <div className="hidden md:block text-sm text-[var(--ink-muted)] tabular">{formatDate(exp.date || exp.expense_date)}</div>
                ...
                <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={(e) => { e.stopPropagation(); setModal(exp); }} className="h-7 w-7 rounded-full flex items-center justify-center text-[var(--ink-muted)] hover:bg-[var(--surface)] hover:text-[var(--ink)] cursor-pointer"><Pencil size={13} /></button>
                  <button onClick={(e) => onDelete(e, exp)} className="h-7 w-7 rounded-full flex items-center justify-center text-[var(--ink-muted)] hover:bg-[var(--surface)] hover:text-[var(--danger)] cursor-pointer"><Trash2 size={13} /></button>
                </div>
              </div>
            ))}
          </div>
  ...
  useEffect(() => {
    if (open) {
      setForm({
        ...
        expense_date: toDateInput(expense?.date || expense?.expense_date) || new Date().toISOString().slice(0, 10),

...
export function InvoiceEditor({ invoiceId }) {
  const params = useParams();
  const id = invoiceId || params?.id;
  const isEdit = !!id && id !== "new";
  const router = useRouter();
  const searchParams = useSearchParams();
  const preselectClient = searchParams ? searchParams.get("client") || "" : "";

  const { data: clients } = useClients();
  const { data: settings } = useSettings();
  const { data: existing, isLoading: loadingInvoice } = useInvoice(isEdit ? id : null);
    ...
    if (isEdit) {
      if (existing && !form) {
        setForm({
          ...
          tax_rate: Number(existing.tax_rate || existing.tax) || 0,
    ...
    <div className="max-w-[1100px]">
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="h-9 w-9 rounded-full flex items-center justify-center border border-[var(--border)] bg-[var(--surface)] text-[var(--ink-muted)] hover:text-[var(--ink)] shadow-card cursor-pointer"
          >
            <ArrowLeft size={16} />
          </button>
          <div>
              ...
              {form.items.map((it, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="h-8 w-8 rounded-full flex items-center justify-center text-[var(--ink-muted)] hover:text-[var(--danger)] hover:bg-[var(--surface-2)] justify-self-end cursor-pointer"
                    title="Remove line"
                  >
                    <Trash2 size={14} />
                  </button>
...
function NoteField({ label, value, onChange, placeholder, aiKind, aiContext }) {
  ...
  async function writeWithAI() {
    setLoading(true);
    try {
      const res = await aiApi.invoiceNotes({
        projectType: aiContext?.items?.[0]?.description || "freelance services",
        terms: value?.trim() || "Net 15 days",
      });
      if (res?.notes) {
        onChange(res.notes);
      }
    ...
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        <button
          ...
          onClick={writeWithAI}
          disabled={loading}
          className="inline-flex items-center gap-1 text-[11px] font-semibold text-[var(--accent-strong)] hover:underline disabled:opacity-50 cursor-pointer"
        >
        ...
        <div className="absolute right-0 top-10 z-20 w-64 max-h-72 overflow-y-auto rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-hover p-1.5">
          {items.map((it) => (
            <button
              ...
              className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl text-left hover:bg-[var(--surface-2)] transition-colors cursor-pointer"
            >
